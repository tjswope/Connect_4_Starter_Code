...
def minimax(current_board, depth, max_player, color, evaluate_algorothm):
  ...
  if depth == 0 or current_board.winner(RED) != False or current_board.winner(WHITE) != False:
    return evaluate_algorothm.evaluate(current_board), current_board

  if max_player:
    maxEval = float('-inf')
    best_move = None
    for move in get_all_moves(current_board, WHITE):
      evaluation = minimax(move, depth-1, False, RED, evaluate_algorothm)[0]
    ...
    for move in get_all_moves(current_board, RED):
      evaluation = minimax(move, depth-1, True, WHITE, evaluate_algorothm)[0]
